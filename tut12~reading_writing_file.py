...
7. "+" : This mode is used to update a file(read+write mode)
...
9. with block - If we open a file with the help of with block we need to worry about closing it as it gets closed
...
# my_next = open('practice_text_file.txt')
...
# other way of printing all the lines in a text file is by iterating on the pointer and not on the container(content)
